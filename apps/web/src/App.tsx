...
const api = axios.create({ baseURL: apiBase || undefined }); // falls back to relative
    ...
    <main style={{ ...pageStyle, paddingBottom: 48 }}>
      <div style={{ maxWidth: 720, margin: "2rem auto", fontFamily: "system-ui" }}>
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <h1 style={{ margin: 0 }}>Zustand + React Query</h1>
          <button
        ...
        <section style={{ marginTop: 12 }}>
          <p style={{ opacity: 0.8 }}>
            Your name and theme are stored in <strong>Zustand</strong> (and persist across refreshes).
          </p>
        ...
        <section style={{ display: "grid", gap: 8 }}>
          ...
          {error && (
            <div style={{ color: "crimson", padding: 12, border: "1px solid #faa", borderRadius: 8 }}>
              Couldn’t reach API. Is it running? Check VITE_API_URL in apps/web/.env.
            </div>
          )}
        ...
        <footer style={{ marginTop: 24, fontSize: 12, opacity: 0.7 }}>
          API base: <code>{apiBase || "(relative)"}</code>
        </footer>
      </div>
    </main>
  );
